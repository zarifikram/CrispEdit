    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            ...
            if 't5' in self.model_name.lower():
                ...
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, cache_dir="/data0/zikram/huggingface/hub/", **model_kwargs)
                self.tok = AutoTokenizer.from_pretrained(self.model_name, cache_dir="/data0/zikram/huggingface/hub/", use_fast=False)
                self.tok.pad_token_id = self.tok.eos_token_id
            ...
            if self.alg_name == 'IKE':
                ...
                if 'locality' in all_metrics[idx]['post'].keys():
                    for locality_key in request['locality'].keys():
                        locality_result = []
                        if hasattr(self.hparams, 'evaluation_type') and self.hparams.evaluation_type == "WILD":
